...
async function loadPublicGames(){
  ...
  // Trending
  try{
    const r=await fetch('https://api.rawg.io/api/games?key='+RAWG+'&dates='+fmt(monthAgo)+','+fmt(today)+'&ordering=-added&page_size=10');
    const data=await r.json();
    const el=document.getElementById('trendingGames');
    if(!el)return;
    if(!data.results||!data.results.length){el.innerHTML='<div style="color:var(--muted);grid-column:1/-1;padding:20px">Sonuç bulunamadı.</div>';return;}
    el.innerHTML='';
    data.results.forEach((game,idx)=>{
      const year=game.released?game.released.slice(0,4):'';
      const mc=game.metacritic;
      const mcColor=mc?(mc>=75?'#6fcf6f':mc>=50?'#ffd166':'#ff6b6b'):'';
      ...
      card.onmouseenter=()=>{card.style.transform='translateY(-4px)';card.style.borderColor='var(--accent)';if(card.querySelector('img'))card.querySelector('img').style.transform='scale(1.05)';};
      card.onmouseleave=()=>{card.style.transform='';card.style.borderColor='var(--border)';if(card.querySelector('img'))card.querySelector('img').style.transform='';};
      card.onclick=()=>openTrailer(game);
      const rankBadge='<div style="position:absolute;top:8px;left:8px;background:rgba(0,0,0,.85);color:#e8ff47;font-family:Bebas Neue,sans-serif;font-size:20px;width:32px;height:32px;border-radius:8px;display:flex;align-items:center;justify-content:center;z-index:2;border:1px solid #e8ff47">'+(idx+1)+'</div>';
      const imgHtml=game.background_image?'<div style="width:100%;aspect-ratio:16/9;overflow:hidden;"><img src="'+game.background_image+'" style="width:100%;height:100%;object-fit:cover;display:block;transition:transform .3s" loading="lazy"></div>':'<div style="width:100%;aspect-ratio:16/9;background:var(--surface2);display:flex;align-items:center;justify-content:center;font-size:32px">🎮</div>';
      card.innerHTML=rankBadge+imgHtml+
        '<div style="padding:12px">'+
          ...
          '<div style="display:flex;align-items:center;justify-content:space-between">'+
            ...
            '<a href="'+steamUrl+'" target="_blank" rel="noopener" onclick="event.stopPropagation()" style="display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:5px;background:rgba(23,26,33,.9);border:1px solid rgba(102,192,244,.2);text-decoration:none;"><svg width="12" height="12" viewBox="0 0 496 512"><path d="M496 256c0 137-111.2 248-248.4 248-113.8 0-209.7-76.3-239.1-180.4l95.2 39.3c6.4 32.1 34.9 56.4 68.9 56.4 39.2 0 71.9-32.4 70.2-73.5l84.5-60.2c52.1 1.3 95.8-40.9 95.8-93.5 0-51.6-42-93.5-93.7-93.5s-93.7 42-93.7 93.5v1.2L176.6 279c-15.5-.9-30.7 3.4-43.5 12.1L0 235.1C10.2 103.7 122.2 8 248 8c136.9 0 248 111 248 248z" fill="#66c0f4"/></svg></a>'+
          '</div>'+
        '</div>';
      el.appendChild(card);
    });
  }catch(e){const el=document.getElementById('trendingGames');if(el)el.innerHTML='<div style="color:var(--muted);grid-column:1/-1;padding:20px">Yüklenemedi: '+e.message+'</div>';}

  // Most added - load from Supabase (may fail if not logged in due to RLS - show placeholder)
  try{
    const SB_URL='https://dzjmbwyandrkonqdjalk.supabase.co';
    const SB_KEY='sb_publishable_oeokT78grwbNQeKo8xtZRA_xJdBkuPD';
    const {createClient}=supabase;
    const sbPub=createClient(SB_URL,SB_KEY);
    const {data:rows}=await sbPub.from('game_lists').select('game_id,game_name,game_image,score');
    const el2=document.getElementById('mostAddedGames');
    if(!el2)return;
    if(!rows||!rows.length){el2.innerHTML='<div style="color:var(--muted);grid-column:1/-1;padding:20px">Henüz oyun eklenmedi.</div>';return;}
    const map={};
    rows.forEach(r=>{if(!map[r.game_id])map[r.game_id]={name:r.game_name,image:r.game_image,count:0,scores:[]};map[r.game_id].count++;map[r.game_id].scores.push(r.score);});
    const sorted=Object.values(map).sort((a,b)=>b.count-a.count).slice(0,10);
    el2.innerHTML='';
    sorted.forEach((g,idx)=>{
      const avg=(g.scores.reduce((s,x)=>s+x,0)/g.scores.length).toFixed(1);
      const card=document.createElement('div');
      card.style.cssText='background:var(--surface);border:1px solid var(--border);border-radius:12px;overflow:hidden;transition:all .25s;position:relative;';
      card.onmouseenter=()=>{card.style.transform='translateY(-4px)';card.style.borderColor='var(--accent)';};
      card.onmouseleave=()=>{card.style.transform='';card.style.borderColor='var(--border)';};
      const rankBadge='<div style="position:absolute;top:8px;left:8px;background:rgba(0,0,0,.85);color:#e8ff47;font-family:Bebas Neue,sans-serif;font-size:20px;width:32px;height:32px;border-radius:8px;display:flex;align-items:center;justify-content:center;z-index:2;border:1px solid #e8ff47">'+(idx+1)+'</div>';
      const imgHtml=g.image?'<div style="width:100%;aspect-ratio:16/9;overflow:hidden;"><img src="'+g.image+'" style="width:100%;height:100%;object-fit:cover;display:block" loading="lazy"></div>':'<div style="width:100%;aspect-ratio:16/9;background:var(--surface2);display:flex;align-items:center;justify-content:center;font-size:32px">🎮</div>';
      card.innerHTML=rankBadge+imgHtml+
        '<div style="padding:12px">'+
          '<div style="font-size:13px;font-weight:600;margin-bottom:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+g.name+'</div>'+
          '<div style="display:flex;align-items:center;justify-content:space-between">'+
            '<span style="font-size:11px;color:var(--muted)">'+g.count+' kullanıcı</span>'+
            '<span style="font-size:13px;font-weight:700;color:#e8ff47">★ '+avg+'</span>'+
          '</div>'+
        '</div>';
      el2.appendChild(card);
    });
  }catch(e){const el2=document.getElementById('mostAddedGames');if(el2)el2.innerHTML='<div style="color:var(--muted);grid-column:1/-1;padding:20px">Henüz veri yok.</div>';}
}

function openTrailer(game){
  const query=encodeURIComponent(game.name+' official trailer');
  ...
  modal.innerHTML='<div style="background:#13131a;border:1px solid #2a2a38;border-radius:16px;overflow:hidden;width:100%;max-width:700px;">'+
    '<div style="display:flex;align-items:center;justify-content:space-between;padding:14px 20px;border-bottom:1px solid #2a2a38;">'+
      ...
      '<button onclick="document.getElementById('trailerModal').remove()" style="background:none;border:none;color:#666;font-size:22px;cursor:pointer;line-height:1">✕</button>'+
    '</div>'+
    '<div style="padding:24px;display:flex;flex-direction:column;align-items:center;gap:16px;">'+
      (game.background_image?'<img src="'+game.background_image+'" style="width:100%;border-radius:8px;max-height:240px;object-fit:cover" loading="lazy">':'')+
      '<div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:center;">'+
        ...
        '<a href="https://store.steampowered.com/search/?term='+encodeURIComponent(game.name)+'" target="_blank" rel="noopener" style="background:#1b2838;color:#66c0f4;text-decoration:none;padding:10px 24px;border-radius:8px;font-weight:700;font-size:14px;border:1px solid #66c0f4">Steam Sayfası</a>'+
      '</div>'+
    '</div>'+
  '</div>';
...
// Auto-load public games on page load
document.addEventListener('DOMContentLoaded', loadPublicGames);
